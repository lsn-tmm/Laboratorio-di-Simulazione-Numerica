...
double variationalWaveFunction_second(double v)
{
  ...
  double exp_m = exp( -pow(v-mu,2) / (2.*pow(sigma,2)) );
  double exp_p = exp( -pow(v+mu,2) / (2.*pow(sigma,2)) );

  return ( (pow(v,2) - 2.*v*mu + pow(mu,2) - pow(sigma,2)) * exp_m + (pow(v,2) + 2.*v*mu + pow(mu,2) - pow(sigma,2)) * exp_p ) / pow(sigma,4);
  //return 0;
}
