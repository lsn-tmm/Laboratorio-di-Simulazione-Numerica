...
class Individual{
 ...
 Individual() : _size(32), _l1(100) { for(uint i=0;i<_size;i++) _DNA.push_back(i); }
  Individual(std::vector<int>::iterator it, std::vector<int>::iterator it2) { _DNA = vector<int>(it,it2); _size = _DNA.size(); _l1=100.; _fitness=0;}
  ...
  Individual & operator=(const Individual & mig){
    (*this)._size = mig._size;
    (*this)._l1 = mig._l1;
    (*this)._fitness = mig._fitness;
    (*this)._DNA = mig._DNA;
...
class Population{
 ...
 void set_best(Individual a) { _best = a; }
